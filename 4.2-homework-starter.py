...
exercise1_results = collection.get(
    where={"category": "vpn"}
)
print_results("Exercise 1 results", exercise1_results)
...
exercise2_results = collection.get(
    where={
        "$and": [
            {"priority": "high"},
            {"year": 2025},
...
print_results("Exercise 2 results", exercise2_results)
...
exercise3a_results = collection.get(
    where_document={"$contains": "student"}
)
print_results('Exercise 3A results ("student")', exercise3a_results)

exercise3b_results = collection.get(
    where_document={
        "$and": [
            {"$contains": "student"},
...
print_results('Exercise 3B results ("student" and not "password")', exercise3b_results)
...
# Write your code here:
# Baseline: semantic search without filters (for distance comparison)
exercise4_unfiltered = collection.query(
    query_texts=["how do I print documents on campus"],
    n_results=5,
)
print_results("Exercise 4 baseline (unfiltered)", exercise4_unfiltered, show_distances=True)

# Filtered: category == printing AND document contains "page"
exercise4_filtered = collection.query(
    query_texts=["how do I print documents on campus"],
    n_results=5,
    where={"category": "printing"},
    where_document={"$contains": "page"},
)
print_results("Exercise 4 filtered (printing + contains 'page')", exercise4_filtered, show_distances=True)